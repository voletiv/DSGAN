...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        ...
        if target_is_real:
            ...
            if create_label:
                self.real_label_var = torch.ones(input.size()).fill_(self.real_label).to(self.device)
            target_tensor = self.real_label_var
        else:
            create_label = ((self.fake_label_var is None) or
                            (self.fake_label_var.numel() != input.numel()))
            if create_label:
                self.fake_label_var = torch.ones(input.size()).fill_(self.fake_label).to(self.device)
...
class VGGLoss(nn.Module):
    def __init__(self, gpu_ids, normalize=False, device='cpu'):
        ...
        if self.normalize:
            ...
            self.mean_img = Variable(mean_img.to(device))
            self.std_img = Variable(std_img.to(device))
